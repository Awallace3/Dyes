...
def filemaker(filename,smi,inchlkey,name,type):
    if type == 'backbone':
        with open(str(filename+1)+'b.smi','w+') as fp:
            fp.write(str(smi))
            fp.write('\n')
            fp.write(str(name))
            fp.write('\n')
            fp.write(str(inchlkey))
            fp.close()
    if type == 'donor':
        with open(str(filename+1)+'ed.smi','w+') as fp:
            fp.write(str(smi))
            fp.write('\n')
            fp.write(str(name))
            fp.write('\n')
            ...
            fp.close()
    if type=='acceptor':
        with open(str(filename+1)+'ea.smi','w+') as fp:
            fp.write(str(smi))
            fp.write('\n')
            fp.write(str(name))
            fp.write('\n')
...
def main():
    ...
    path_to_acceptors = 'eAcceptors'
    path_to_donors = 'trieDonors'
    path_to_backbone = 'tribackbones'
    name = input('What is name of chemdraw stucture without the Kr and Ar: ')
  #  name = 'ffff'
    smile = input('What is the smile string from Chemdraw (Place Ar for electron acceptor connection and Kr for electron donor connection): ')
  #  smile = 'CCC[Kr]CCC[Ar]'
    ...
    smi = arkysmiconv(smile)
    struct,numoffiles = typeofstruct(path_to_acceptors,path_to_donors,path_to_backbone,smi)
    print((struct,'AAAAA'))
    filemaker(numoffiles,smi,inchlkey,name,struct)
    '''
