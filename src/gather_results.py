...
def json_pandas_molecule_BM(path_results, exc_json=False):
    ...
    if exc_json:
        ...
        'name', 'parts',
        'generalSMILES','localName',
        'SMILES', 'excitations', 'exp'
        ]]
        df = pd.json_normalize(data=dat['molecules'], record_path='excitations',
                                meta=['name', 'SMILES', 'generalSMILES','localName', 'parts', 'exp' ])
        df = df [[
...
def df_molecules_to_df_method_basisset_exc(df_molecules, method_basis_set=[], exp=False, band_gap=False):
        ...
        if str(r1['name']) not in Names:
            ...
            for n, j in enumerate(method_basis_set_lst):
                ...
                if band_gap:
                    if str(r1['method_basis_set']) in j and 'Band Gap' in j:
                        method_basis_set_lst[n] = round(r1['HOMO'] - r1['LUMO'], 3)
            ...
            for j in method_basis_set:
                if str(j) == r1['method_basis_set']:
                    if r1['exc'] == 1:
                        if exp:
                            df.loc[df['Name'] == r1['name'], [j, 'Exp']] = [conv_energy(r1['nm']), conv_energy(r1['exp'])]
                        else:
                            df.loc[df['Name'] == r1['name'], [j]] = [conv_energy(r1['nm'])]
                        df.loc[df['Name'] == r1['name'], ["HOMO %s"%j]] = round(r1['HOMO'], 3)
                        df.loc[df['Name'] == r1['name'], ["LUMO %s"%j]] = round(r1['LUMO'], 3)
                        if band_gap:
                            df.loc[df['Name'] == r1['name'], ["Band Gap %s"%j]] = round(r1['HOMO'] - r1['LUMO'], 3)
...
def theoretical_dyes_basis_set_out(
    ...
    if units.lower() == 'ev':
        ...
        # df_molecules = df_conv_energy(df_molecules)
    if output_csv != '':
        if homo_lumo:
...
def benchmarks_dyes_basis_set_out(
    ...
    df_molecules = json_pandas_molecule_BM(path_results_json, exc_json=exc_json)
    # print(df_molecules)
    df = df_molecules_BM_to_df_method_basisset(df_molecules, methods_basissets)
    convert_lst = methods_basissets.copy()
    convert_lst.append("Exp")
