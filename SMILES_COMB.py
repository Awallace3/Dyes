...
def number_gather(smi):
   # print(smi[0])
    ...
    for num,i in enumerate(smi[0]):
        if smi[0][num]=='%' or smi[0][num-1]=='%' or smi[0][num-2]=='%':
            edit = str(i).replace('%','*')
            if '*' in edit:
    ...
    for num,i in enumerate(smi[1]):
        if smi[1][num]=='%' or smi[1][num-1]=='%' or smi[1][num-2]=='%':
            edit = str(i).replace('%','*')
            if '*' in edit:
    ...
    for num,i in enumerate(smi[2]):
        if smi[2][num]=='%' or smi[2][num-1]=='%' or smi[2][num-2]=='%':
            edit = str(i).replace('%','*')
            if '*' in edit:
...
def check(smi,add):
    ...
    for num,i in enumerate(Backbones):
        if Backbones[num]== '%' or Backbones[num-1] == '%' or Backbones[num-2]== '%':
            edit = str(i).replace('%','*')
            if '*' in edit:
            ...
            try:
                ...
                if i+add[1] > 10:
                    ...
                    print('SMILES string value is greater than 100 and further analysis needed.')
                else:
                    i = int(i)+add[1]
                    Backbones_updt += str(i)
    ...
    #print(Backbones_updt)
    for num,i in enumerate(eAcceptor):
        if eAcceptor[num]== '%' or eAcceptor[num-1] == '%' or eAcceptor[num-2]== '%':
            edit = str(i).replace('%','*')
            if '*' in edit:
